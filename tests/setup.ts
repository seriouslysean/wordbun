import { afterEach, vi } from 'vitest';
import type { WordData } from '#types';
import mockTranslations from '#tests/locales/en-us.fixtures.json' with { type: 'json' };

export interface MockEnvironment {
  SITE_TITLE: string;
  SITE_DESCRIPTION: string;
  SITE_ID: string;
  SITE_URL: string;
  SITE_LOCALE: string;
  SITE_KEYWORDS: string;
  SITE_AUTHOR: string;
  SITE_AUTHOR_URL: string;
  SITE_ATTRIBUTION_MESSAGE: string;
  HUMANS_WORD_CURATOR: string;
  HUMANS_DEVELOPER_NAME: string;
  HUMANS_DEVELOPER_CONTACT: string;
  HUMANS_DEVELOPER_SITE: string;
  COLOR_PRIMARY: string;
  COLOR_PRIMARY_LIGHT: string;
  COLOR_PRIMARY_DARK: string;
  COLOR_DARK_PRIMARY: string | undefined;
  COLOR_DARK_PRIMARY_LIGHT: string | undefined;
  COLOR_DARK_PRIMARY_DARK: string | undefined;
  COLOR_DARK_BACKGROUND: string | undefined;
  COLOR_DARK_BACKGROUND_LIGHT: string | undefined;
  COLOR_DARK_TEXT: string | undefined;
  COLOR_DARK_TEXT_LIGHT: string | undefined;
  COLOR_DARK_BORDER: string | undefined;
  GA_ENABLED: boolean;
  GA_MEASUREMENT_ID: string | undefined;
  SENTRY_ENABLED: boolean;
  SENTRY_DSN: string | undefined;
  BASE_PATH: string | undefined;
  SOURCE_DIR: string;
}

export interface MockCollectionEntry {
  id: string;
  data: WordData;
}

declare global {
  var mockEnv: MockEnvironment;
  var mockWordData: MockCollectionEntry[];
}

// Mock astro:env/client with mutable defaults for all tests
// Individual tests can override via mockEnv.FIELD = 'value'
export const mockEnv: MockEnvironment = {
  SITE_TITLE: 'Test Site',
  SITE_DESCRIPTION: 'Test Description',
  SITE_ID: 'test-site',
  SITE_URL: 'https://test.com',
  SITE_LOCALE: 'en',
  SITE_KEYWORDS: 'test,keywords',
  SITE_AUTHOR: 'Test Author',
  SITE_AUTHOR_URL: 'https://test.com',
  SITE_ATTRIBUTION_MESSAGE: 'Test Attribution',
  HUMANS_WORD_CURATOR: '',
  HUMANS_DEVELOPER_NAME: '',
  HUMANS_DEVELOPER_CONTACT: '',
  HUMANS_DEVELOPER_SITE: '',
  COLOR_PRIMARY: '#9a3412',
  COLOR_PRIMARY_LIGHT: '#c2410c',
  COLOR_PRIMARY_DARK: '#7c2d12',
  COLOR_DARK_PRIMARY: undefined,
  COLOR_DARK_PRIMARY_LIGHT: undefined,
  COLOR_DARK_PRIMARY_DARK: undefined,
  COLOR_DARK_BACKGROUND: undefined,
  COLOR_DARK_BACKGROUND_LIGHT: undefined,
  COLOR_DARK_TEXT: undefined,
  COLOR_DARK_TEXT_LIGHT: undefined,
  COLOR_DARK_BORDER: undefined,
  GA_ENABLED: false,
  GA_MEASUREMENT_ID: undefined,
  SENTRY_ENABLED: false,
  SENTRY_DSN: undefined,
  BASE_PATH: '/',
  SOURCE_DIR: '',
};

vi.mock('astro:env/client', () => mockEnv);

// Expose for tests that need to override env values
globalThis.mockEnv = mockEnv;

// Each spec file gets its own module registry and mockEnv object (vitest
// isolates files), but the tests within a file share them. Tests that
// override BASE_PATH (url-utils, WordLink, the page-metadata wrapper) would
// otherwise leak the value into the next test in the same file. Restore the
// default after every test so each starts from a clean BASE_PATH.
afterEach(() => {
  mockEnv.BASE_PATH = '/';
});

// Mock fixture data for testing
const mockWordData: MockCollectionEntry[] = [
  {
    id: '2025/20250121',
    data: {
      word: 'occasional',
      date: '20250121',
      adapter: 'wordnik',
      data: [
        {
          text: 'Occurring or appearing at irregular or infrequent intervals.',
          partOfSpeech: 'adjective',
          sourceDictionary: 'wordnik',
        },
      ],
    },
  },
  {
    id: '2025/20250120',
    data: {
      word: 'word',
      date: '20250120',
      adapter: 'wordnik',
      data: [
        {
          text: 'A unit of language.',
          partOfSpeech: 'noun',
          sourceDictionary: 'wordnik',
        },
      ],
    },
  },
  {
    id: '2025/20250119',
    data: {
      word: 'serendipity',
      date: '20250119',
      adapter: 'wordnik',
      data: [
        {
          text: 'The faculty of making fortunate discoveries by accident.',
          partOfSpeech: 'noun',
          sourceDictionary: 'wordnik',
        },
      ],
    },
  },
];

// Mock astro:content module
vi.mock('astro:content', () => ({
  getCollection: vi.fn().mockImplementation((collectionName: string) => {
    if (collectionName === 'words') {
      return Promise.resolve(mockWordData);
    }
    return Promise.resolve([]);
  }),
}));

// Mock translations with educational/word-focused fixtures
vi.mock('#locales/en.json', () => ({ default: mockTranslations }));

// Expose mock data for tests that need it
globalThis.mockWordData = mockWordData;
