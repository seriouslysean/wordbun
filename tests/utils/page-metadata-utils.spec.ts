...
declare const mockEnv: Record<string, string | boolean | undefined>;

vi.mock('#utils/word-stats-utils', async importOriginal => {
  const actual = await importOriginal<typeof import('#utils/word-stats-utils')>();
  return {
    ...actual,
...
// Test data
const makeWord = (word: string, date: string): WordData => ({
  word,
  date,
  adapter: 'test',
...
const mockWords = [
  makeWord('test', '20240101'),
  makeWord('example', '20240102'),
  makeWord('sample', '20240103'),
];

describe('page-metadata-utils', () => {
  it('reuses corpus stats for a lookup while custom datasets stay fresh', () => {
    const words = [makeWord('test', '20240101')];
    ...
    const customWords = [makeWord('one', '20240101')];
    expect(getPageMetadata('/word', customWords).secondaryText).toBe('1 Mock Items');
    customWords.push(makeWord('two', '20240102'));
