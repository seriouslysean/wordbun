...
const ctx = { root: '', env: {}, engine: '', site: '', origin: '' };

const git = (cwd, ...args) => execFileSync('git', args, { cwd, env: ctx.env, encoding: 'utf-8', stdio: 'pipe' }).trim();
...
const gatesRun = () => npmCalls().map(line => line.split('\t')[0]);

const syncBranches = () => git(ctx.site, 'branch', '--list', 'sync/*', '--format=%(refname:short)');
const expectedBranch = () => `sync/upstream-${git(ctx.site, 'rev-parse', '--short', 'upstream/main')}`;
const originRefs = () => git(ctx.origin, 'for-each-ref', '--format=%(refname) %(objectname)');
const parents = ref => git(ctx.site, 'rev-list', '--parents', '-n', '1', ref).split(' ').slice(1);
const isMerging = () => fs.existsSync(path.join(ctx.site, '.git', 'MERGE_HEAD'));
...
const tempDir = () => fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'wotd-sync-')));
...
describe('sync-upstream.sh', { timeout: 90000 }, () => {
  describe('refuses before fetching or branching', () => {
    it.each([
      ...
      ['an untracked file', () => write(ctx.site, 'data/words/2026/20260102.json', '{}\n')],
