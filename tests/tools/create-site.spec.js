...
const run = (...args) => spawnTool([TOOL, ...args], { cwd: ctx.dir });
...
const files = dir => fs.readdirSync(path.join(ctx.dir, dir), { recursive: true, withFileTypes: true })
  .filter(entry => entry.isFile())
  .map(entry => path.relative(path.join(ctx.dir, dir), path.join(entry.parentPath, entry.name)))
  .toSorted();

// The caller YAML blocks docs/technical.md shows under Site Repositories
const documentedCaller = workflow => {
...
describe('create-site', { timeout: 20000 }, () => {
  ...
  it('groups engine updates for Dependabot', async () => {
    ...
    expect(config.updates).toEqual([expect.objectContaining({
