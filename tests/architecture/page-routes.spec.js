...
const isRest = name => /^\[\.\.\.[^\]]+\]$/.test(name);
const isParam = name => /^\[[^\]]+\]$/.test(name) && !isRest(name);
...
const resolvesToPage = (dir, segments) => {
  ...
  const byRest = () => pages.some(isRest)
...
const isRouted = (root, urlPath) => resolvesToPage(root, urlPath.split('/').filter(Boolean));
